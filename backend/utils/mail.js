...
async function SendFollowMail(follower, email) {
  const transporter = nodemailer.createTransport({
    service: 'gmail',
    auth: {
      user: `${process.env.email}`,
  ...
  await transporter.sendMail({
    from: 'Insta-clone',
    to: [`${email}`],
    subject: `${follower} started following you`,
    text: `${follower} started following you`,
    html: `<p>${follower} started following you.</p>`,
  });
}

module.exports = { SendMail, SendFollowMail };