...
exports.homePosts = async (req, res) => {
  try {
    ...
    if (user && user.followings.length > 0) {
      // Carga los posts de los usuarios que sigues
      const followPosts = await Promise.all(
        user.followings.map(async (item) => {
          return await Post.find({ owner: item });
        })
      );
      followPosts.forEach(fp => posts.push(...fp));
    }
    // Ordena por fecha descendente
    const arr = posts.sort((a, b) => b.createdAt - a.createdAt);
    res.status(200).json(arr); // <-- SIEMPRE responde un array
  } catch (err) {
    res.status(200).json([]); // <-- Aquí el fix principal, responde SIEMPRE un array aunque haya error
  }
};
